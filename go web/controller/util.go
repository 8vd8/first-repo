...
func sendEMail(target, subject, content string) {
	server, port, usr, pwd := config.GetSMTPfig()
	d := gomail.NewDialer(server, port, usr, pwd)
	//Dialer实例用于连接SMTP服务器
	d.TLSConfig = &tls.Config{InsecureSkipVerify: true}
	//表示跳过对服务器验证书的验证
	m := gomail.NewMessage() //创建邮件消息对象
...
func setSessionUser(w http.ResponseWriter, r *http.Request, username string) error {
	...
	session.Values["user"] = username //Values是map[interface{}]interface{}类型的
...
func getFlash(w http.ResponseWriter, r *http.Request) string {
	session, _ := store.Get(r, sessionName)
	fm := session.Flashes(flashName) //显示会话信息
	if fm == nil {
		return ""
	}
	session.Save(r, w) //表示已读，flash消息是一次性，下一次不会再显示
	return fmt.Sprintf("%v", fm[0])
}

func getPage(r *http.Request) int {
	url := r.URL         //得到URL对象
	query := url.Query() //从URL对象中获取名为“page”的查询参数的值
	q := query.Get("page")
	if q == "" {
		return 1
	}
...
func checkEmailExistRegister(email string) string {
	if vm.CheckUserExist(email) {
		return fmt.Sprintf("Email has registered by other,please choosse another email.")
	}
	return ""
}
