...
func (u *User) FollowingPostsByPageAndLimit(page, limit int) (*[]Post, int, error) {
	...
	if err := db.Preload("User").Order("timestamp desc").Where("user_id in (?)", ids).Offset(offset).Limit(limit).Find(&posts).Error; err != nil {
		//offset用来指明一页的偏移操作（用人话来说就是：就是从哪里开始）
		//比如说：第一页从第一行数据开始，偏移量为0，第二页从第十行数据开始，偏移量为10
		//offset和limit通常一起使用
...
func (u *User) GenerateToken() (string, error) { //返回一个JWT令牌（字符串类型）
...
func CheckToken(tokenString string) (string, error) {
	...
	}) //解析JWT令牌，第一个参数是解析令牌字符串，第二个参数是一个函数，用于验证签名的密匙
	//主要是验证签名的算法是否是预期的HMAC算法
...
func (u *User) FormattedLastSeen() string {
	return u.LastSeen.Format("2006-01-02 15:04:05") //按照这样的格式来书写后端
}
